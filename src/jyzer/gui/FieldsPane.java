...
import jyzer.gui.guihelpers.FieldsTableModel;
import jyzer.items.fields.FieldInfo;

/**
 * This class is the representative of <code>Fields</code> in the <code>ClassFile</code>. You can search a String
 * in the displayed data, and view the attributes of a field if any is given.
 *
 * @author Legendi Richard Oliver
 * @version 1.0
 */
public class FieldsPane extends JComponent {
	...
	/**
	 * Constructor, creates a new instance of FieldsPane. The parent is needed to be able
	 * to make the AttribDialog a modal Dialog.
	 *
	 * @param parentFrame the frame that contains this component in it.
	 */
	public FieldsPane(JFrame parentFrame) {
		super();
		...
		rowSM.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent e) {
				...
				if ( lsm.isSelectionEmpty() ) {
					...
					FieldInfo finfo = model.getField(selectedRow);
					if ( finfo.hasAttributes() ) {
						attributesButton.setEnabled(true);
						attributesLabel.setText( "" + finfo.getAttributesCount() );
					} else {
	...
	private void setDefaultValues() {
		...
		findPanel.setToolTipText("<HTML><B>Search String:</B><BR>To find a string in the field entries.</HTML>");
		attributesPanel.setToolTipText("<HTML><B>Attributes:</B><BR>To display the attributes of the selected field.</HTML>");
	}
	 ...
	 *
	 * @param fields references to the field infos (to know the attributes).
	 * @param data the array of strings to be displayed in the table.
	 */
	public void setData(FieldInfo[] fields, Object[][] data) {
		findTextField.setText("");
		table.setModel( new FieldsTableModel(fields, data) );
		actIndex = -1;
		refresh();
	}
	...
	private void displayAttribs() {
		...
		model.getField( table.getSelectedRow() ).getAttributes().getHTMLDescription(sb);
	...
	private JTable table = new JTable( new FieldsTableModel() );
